...
Y.onmessage=function(aC){f(l);
if(!l.enableXDR&&aC.origin!==window.location.protocol+"//"+window.location.host){c.atmosphere.log(l.logLevel,["Origin was not "+window.location.protocol+"//"+window.location.host]);
return
...
Y.onerror=function(aB){clearTimeout(l.id);
...
}if(l.connectTimeout>0){l.id=setTimeout(function(){if(!az){var aC={code:1002,reason:"",wasClean:false};
am.onclose(aC);
try{g()
}catch(aD){}return
}},l.connectTimeout)
}am.onopen=function(aD){f(l);
...
am.onerror=function(aC){clearTimeout(l.id);
if(l.heartbeatTimer){clearTimeout(l.heartbeatTimer)
}};
am.onclose=function(aC){if(ai.state==="closed"){return
}clearTimeout(l.id);
var aD=aC.reason;
...
break
}}if(l.logLevel==="warn"){c.atmosphere.warn("Websocket closed, reason: "+aD);
...
}else{if(l.reconnect&&ai.transport==="websocket"&&aC.code!==1001){g();
...
if(l.logLevel==="warn"){c.atmosphere.warn("Websocket error, reason: "+aC.reason)
}M(0,"maxReconnectOnClose reached")
}}}}};
...
}}function o(aE,aD){var aC=aD;
if(aE.transport==="polling"){return aC
}if(c.trim(aD).length!==0&&aE.enableProtocol&&aE.firstMessage){var aF=aE.trackMessageLength?1:0;
var aB=aD.split(aE.messageDelimiter);
if(aB.length<=aF+1){return aC
}aE.firstMessage=false;
aE.uuid=c.trim(aB[aF]);
if(aB.length<=aF+2){c.atmosphere.log("error",["Protocol data not sent by the server. If you enable protocol on client side, be sure to install JavascriptProtocol interceptor on server side.Also note that atmosphere-runtime 2.2+ should be used."])
}var ay=parseInt(c.trim(aB[aF+1]),10);
ac=aB[aF+2];
if(!isNaN(ay)&&ay>0){var aA=function(){p(ac);
aE.heartbeatTimer=setTimeout(aA,ay)
};
aE.heartbeatTimer=setTimeout(aA,ay)
}b=false;
if(aE.transport!=="long-polling"){R(aE)
}c.atmosphere.uuid=aE.uuid;
aC="";
aF=aE.trackMessageLength?4:3;
if(aB.length>aF+1){for(var az=aF;
az<aB.length;
az++){aC+=aB[az];
if(az+1!==aB.length){aC+=aE.messageDelimiter
}}}if(aE.ackInterval!==0){setTimeout(function(){p("...ACK...")
},aE.ackInterval)
}}else{if(aE.enableProtocol&&aE.firstMessage&&c.browser.msie&&+c.browser.version.split(".")[0]<10){c.atmosphere.log(l.logLevel,["Receiving unexpected data from IE"])
}else{R(aE)
}}return aC
}function f(ay){ay.timedOut=false;
clearTimeout(ay.id);
if(ay.timeout>0&&ay.transport!=="polling"){ay.id=setTimeout(function(){ay.timedOut=true;
at(ay);
x();
g()
},ay.timeout)
...
if(az.reconnect&&ao++<az.maxReconnectOnClose){ai.ffTryingReconnect=true;
...
}if(!az.reconnectOnServerError&&(aD>=300&&aD<600)){M(aD,aA.statusText);
return
}if(aD>=300||aD===0){ay();
return
...
if(c.trim(aH).length===0&&az.transport==="long-polling"){if(!aA.hasData){ae(aA,az,az.pollingInterval)
...
ay.onload=function(){if(l.timedOut){l.timedOut=false;
g();
az.lastIndex=0;
if(az.reconnect&&ao++<az.maxReconnectOnClose){O("re-connecting",aA.transport,aA);
aC()
}else{M(0,"maxReconnectOnClose reached")
}}};
...
ai.state=ay;
aa()
}function A(ay,aA){if(!aA.readResponsesHeaders){if(!aA.enableProtocol){aA.uuid=c.atmosphere.guid()
}}else{try{var az=ay.getResponseHeader("X-Atmosphere-tracking-id");
if(az&&az!=null){aA.uuid=az.split(" ").pop()
}}catch(aB){}}}function ad(ay){i(ay,l);
i(ay,c.atmosphere)
...
if(an==null&&J!=null){J(ai.responseBody)
}if((ai.responseBody.length===0||(ay&&ac===ai.responseBody))&&ai.state==="messageReceived"){continue
}ad(ai);
...
if(l.connectTimeout>0){c.ajax({url:ay,async:l.closeAsync,timeout:l.connectTimeout,cache:false,crossDomain:l.enableXDR})
}else{c.ajax({url:ay,async:l.closeAsync,cache:false,crossDomain:l.enableXDR})
